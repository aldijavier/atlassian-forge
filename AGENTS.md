...
You are building apps designed to be installed into a single customer site (Terbang Ventures). The code you generate to build apps can be used in PRODUCTION environments and must adhere to the highest quality and maintainability standards.

# Repository Structure

This is a monorepo containing multiple Atlassian Forge apps.
- `apps/`: Contains individual Forge apps.
    - `epic-breakdown`: Helps manage epic breakdowns and story relations.
    - `prd-sync-guardian`: Syncs PRD status and ensures consistency.
    - `program-report`: Generates high-level program status reports.
- `docs/`: Shared documentation.
- root scripts: `deploy.sh`, `deploy-app-script.sh` for multi-environment deployments.

# Development Workflow

- **Branching**: Use `feature/<app-name>/<desc>` or `fix/<app-name>/<desc>`.
- **Apps**: Always navigate to `apps/<app-name>` before running Forge CLI commands.
- **Shared Docs**: Refer to root `.md` files for deployment checklists and pipeline setup.
...
### Epic Breakdown
- Uses `DynamicTable` extensively for listing stories.
- Relies on "Work item split" link types ("Split To" / "Split From").
- Frontend is located in `static/epic-breakdown`.

### Program Report
- Global Page app using UI Kit.
- Aggregates data from multiple Epics linked to a "Program".
- Focuses on "Relative Due Dates" and "Health Indicators".

### PRD Sync Guardian
- Focuses on synchronization logic between different issue types.
